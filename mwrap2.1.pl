...
my $NC = "\033[0m";
my $bold = "\e[1m";
# Operating system dependend overwrites
$bin_path = '/usr/local/bin/' if $Config{osname} eq 'linux';    # default video player for Linux
$vpa = 'MPLAYER' if $Config{osname} eq 'linux'; # default video player for Linux
$mcode_pl = $bin_path.'mcode.pl';
# ---------------------------------------------------------------------------------------
...
if (-e $filename) {
    ...
    if ( ! -d $project_dir ) {
        ...
        if ($answer ne 'y') {
            ...
            $answer = question('Type the number of your choice.','numeric',@f);
...
if ($vpa eq 'MPLAYER') {
    ...
    if ( $Config{osname} eq 'linux' ) {
        ...
        die("No VLC start settings implemented for: ". $Config{osname});
    }
    sleep 2;
    use IO::Socket;
    my $host='127.0.0.1';
...
sub question {
    ...
    my $ret;
    print $gray;
    while (defined($answer = ReadKey(0))) {
        #$hexchar = unpack "H*",$answer;
        if ($type eq 'numeric' and $answer =~ /^\d+?$/ and scalar @ans >= $answer) {
            $answer = $ans[$answer-1];
        } else {
            print $answer;
            next;
